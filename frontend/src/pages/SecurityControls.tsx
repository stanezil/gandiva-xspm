...
const SecurityControls: React.FC = () => {
  ...
  const fetchBenchmarkList = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to fetch benchmark data. Please try again later.",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
  ...
  // Helper function to extract controls from AWS benchmark data regardless of structure
  const extractControls = (data: any): BenchmarkControl[] => {
    ...
    // Case 1: Data has a direct controls array at root level
    if (data.controls && Array.isArray(data.controls)) {
      controls.push(...data.controls);
    }
    
    // Case 2: Process all groups recursively
    ...
    // Case 3: Controls might be directly in the root data as an array of objects
    if (Array.isArray(data) && data.length > 0 && data[0].control_id) {
      controls.push(...data);
    }
    
    // Case 4: Some AWS output formats might have each service as a property
    // with a controls array inside
    if (typeof data === 'object' && data !== null) {
      Object.keys(data).forEach(key => {
        const item = data[key];
        if (item && typeof item === 'object') {
          // If this object has a controls array, add those controls
          if (item.controls && Array.isArray(item.controls)) {
            controls.push(...item.controls);
          }
          
          // Also check for nested structures like item.results where each result might be a control
  ...
  const fetchBenchmarkDetail = async (benchmarkId: string) => {
    ...
    try {
      ...
      console.log(`Extracted ${allExtractedControls.length} total controls from the benchmark`);
      ...
      // Special handling for the AWS compliance benchmark structure
      // In this structure, the aws_compliance.benchmark.all_controls group contains nested groups 
      // with controls instead of a direct controls array
      if (transformedData.groups && Array.isArray(transformedData.groups)) {
        // Find the all_controls group for debugging
        const allControlsGroup = transformedData.groups.find(g => g.group_id === 'aws_compliance.benchmark.all_controls');
        if (allControlsGroup) {
          console.log('Found aws_compliance.benchmark.all_controls group structure:', {
        ...
        transformedData.groups = transformedData.groups.map(group => {
          // Special handling for the aws_compliance.benchmark.all_controls group
          if (group.group_id === 'aws_compliance.benchmark.all_controls') {
            ...
            if ((!group.controls || !Array.isArray(group.controls) || group.controls.length === 0) && 
              ...
              console.log('Fixing aws_compliance.benchmark.all_controls group with nested groups');
              ...
              console.log(`Extracted ${groupControls.length} controls from nested groups in aws_compliance.benchmark.all_controls`);
      ...
      if (allExtractedControls.length > 0) {
        ...
        const hasAllControlsGroup = transformedData.groups.some(
          group => group.group_id === 'aws_compliance.benchmark.all_controls'
        );
      ...
      toast({
        title: "Error",
        description: "Failed to fetch benchmark details. Please try again later.",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
  ...
  const handleRunBenchmark = async () => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Benchmark run completed successfully.",
      });
      ...
      toast({
        title: "Error",
        description: "Failed to run benchmark. Please try again later.",
        variant: "destructive"
      });
    } finally {
  ...
  const getServiceFromControl = (control: BenchmarkControl): string => {
    ...
    const serviceMatch = control.control_id?.match(/aws_([a-z0-9]+)_/);
  ...
  const getAffectedResources = (control: BenchmarkControl): string => {
    ...
    control.results.forEach(result => {
      if (result.resource) {
        // Extract the resource identifier from ARN if possible
        const arnParts = result.resource.split('/');
        const resourceId = arnParts.length > 1 ? arnParts[arnParts.length - 1] : result.resource;
