...
logger = logging.getLogger('kubernetes_benchmark_resource')

# MongoDB connection
config = get_config()
client = MongoClient(config.MONGO_URI)
db = client[config.DB_NAME]
kubernetes_benchmark_collection = db["kubernetes-benchmark"]

# Mock data for testing when the real benchmark can't run
MOCK_BENCHMARK_DATA = {
    "group_id": "root_result_group",
    "title": "All Kubernetes Controls",
    "description": "Sample Kubernetes benchmark data for testing",
    "tags": {},
    "summary": {
        "status": {
            "alarm": 25,
    ...
    "groups": [
        {
            "group_id": "kubernetes_compliance.benchmark.sample",
            "title": "Sample Kubernetes Benchmark",
            "description": "Sample Kubernetes benchmark data for testing purposes",
            "tags": {},
            "controls": [
                {
                    "control_id": "sample_control_1",
                    "title": "Sample Kubernetes Control 1",
                    "description": "This is a sample Kubernetes control",
                    "status": "alarm",
                    "reason": "This is a sample control that's in alarm state"
                },
                {
                    "control_id": "sample_control_2",
                    "title": "Sample Kubernetes Control 2",
                    "description": "This is another sample Kubernetes control",
                    "status": "ok",
...
class KubernetesBenchmarkResource(Resource):
    """Resource for Kubernetes compliance benchmark operations"""
    
    @jwt_required()
    def post(self):
        """Run Kubernetes compliance benchmark and store results in MongoDB"""
        ...
        try:
            ...
            logger.info("Running Kubernetes compliance benchmark...")
            
            try:
                # Run the powerpipe benchmark command for Kubernetes compliance
                result = subprocess.run(
                    [
                        "powerpipe", "benchmark", "run", 
                        "kubernetes_compliance.benchmark.all_controls", 
                        "--output", "json",
                ...
                if result.returncode != 0:
                    ...
                    if not result.stderr and result.stdout and result.stdout.strip():
                        ...
                        insert_result = kubernetes_benchmark_collection.insert_one(benchmark_data)
            ...
            return {
                'message': 'Kubernetes compliance benchmark completed successfully',
                'benchmark_id': benchmark_id,
                'timestamp': benchmark_data['timestamp'].isoformat(),
                'summary': {
    ...
    def get(self, benchmark_id=None):
        """Get Kubernetes compliance benchmark results"""
        try:
            if benchmark_id:
                # Get specific benchmark by ID
                benchmark = kubernetes_benchmark_collection.find_one({'_id': ObjectId(benchmark_id)})
                if not benchmark:
                ...
                benchmarks = list(kubernetes_benchmark_collection.aggregate(pipeline))
